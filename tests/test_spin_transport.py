...
conjugate exp(+2i psi) of the physical transport phase exp(-2i psi),
an order-unity Q/U error.

These tests pin the matched pairing through croissant's own analysis
(band-limited duals of a single E-mode) and the transport itself
(gal->FK5 rotation of an E-only sky against a reference that needs only
scalar rotation of E, since E-modes rotate as scalars).
"""
...
def test_gal_to_fk5_rotation_transports_polarization():
    ...
    for ell in range(2, L):
        for emm in range(ell + 1):
            packed[hp.Alm.getidx(LMAX, ell, emm)] = rng.normal() + (
