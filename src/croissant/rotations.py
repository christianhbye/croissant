...
def _nearest_rotation(mat):
    """
    Return the rotation matrix nearest to ``mat`` in the Frobenius norm.

    Euler angles can only describe a rotation. Astropy's topocentric
    matrices carry ~1e-4 of aberration, which is not one, and both Euler
    splits amplify it near their gimbal lock (#152).
    """
...
def rotmat_to_eulerZYX(mat):
    ...
    """
    mat = _nearest_rotation(mat)
    # cos(pitch) from the rest of the row rather than cos(arcsin(m[0, 2])),
    # which turns rounding at gimbal lock into ~1e-8, past the check below
    cb = np.hypot(mat[0, 0], mat[0, 1])
    beta = -np.arctan2(mat[0, 2], cb)  # pitch
...
def rotmat_to_eulerZYZ(mat):
    ...
    """
    mat = _nearest_rotation(mat)
    # sin(beta) from the rest of the column rather than sqrt(1 - m[2, 2]**2),
    # which turns rounding at gimbal lock into ~1e-8, past the checks below
    sin_beta = np.hypot(mat[0, 2], mat[1, 2])
    beta = np.arctan2(sin_beta, mat[2, 2])
