...
import numpy as np
from croissant import coordinates
...
def test_coord_rep():
    ...
    for i in range(len(coords)):
        assert coordinates.coord_rep(coords[i]) == short[i]


def test_get_rot_mat():
    # check that we agree with healpy for galactic -> equatorial
    rot_mat = coordinates.get_rot_mat("galactic", "fk5")
    rot = hp.Rotator(coord=["G", "C"])
    assert np.allclose(rot_mat, rot.mat)

    # equatorial -> galactic
    rot_mat = coordinates.get_rot_mat("fk5", "galactic")
    ...
    rot_mat = coordinates.get_rot_mat("fk5", to_frame)
    x, y, z = np.eye(3)
    xp, yp, zp = (
    ...
    rot_mat = coordinates.get_rot_mat("mcmf", to_frame)
    xp, yp, zp = (
    ...
    rot_mat = coordinates.get_rot_mat("galactic", MCMF())
    xp, yp, zp = (
        SkyCoord(x=x, y=y, z=z, frame="mcmf", representation_type="cartesian")
...
def test_rotmat_to_euler():
    ...
    rot_mat = coordinates.get_rot_mat("galactic", "fk5")
    eul = coordinates.rotmat_to_euler(rot_mat)
